...
# Function to locate Blender 2.93 on Linux
find_blender_linux() {
    echo "Searching for Blender 2.93 on Linux..."

    # Typical locations with version-specific checks
    paths=(
        "/usr/bin/blender"
        "/usr/local/bin/blender"
        "$HOME/blender_versions/blender-2.93*/blender"
        "$HOME/blender/stable/blender-2.93*/blender"
        "/opt/blender/blender-2.93*/blender"
    )

    for path in "${paths[@]}"; do
        if [ -x "$path" ]; then
            # Check if the version is 2.93
            version=$("$path" --version | grep -o "Blender 2.93")
            if [ "$version" == "Blender 2.93" ]; then
                echo "Blender 2.93 found at: $path"
    ...
    echo "Blender 2.93 not found in default locations."
    exit 1
}

# Function to locate Blender 2.93 on Windows
find_blender_windows() {
    echo "Searching for Blender 2.93 on Windows..."

    # Converting Windows paths to Linux format using /mnt/c for Git Bash, Cygwin, or WSL
    paths=(
        "/mnt/c/Program Files/Blender Foundation/Blender 2.93*/blender.exe"
        "/mnt/c/Program Files/Blender Foundation/Blender 2.93*/blender.exe"
        "/mnt/c/Program Files (x86)/Blender Foundation/Blender 2.93*/blender.exe"
        "/mnt/c/Users/$USER/AppData/Local/Blender Foundation/Blender 2.93*/blender.exe"
    )
