...
class BaseScene:
  ...
  def set_cycles_settings(self):
    ...
    bpy.context.scene.cycles.denoising_use_gpu = self.cycles_denoising_use_gpu

  def set_eevee_settings(self):
    bpy.context.scene.eevee.use_shadows = self.eevee_use_shadows
  ...
  def create_planes(self):
    ...
    bpy.context.active_object.active_material.surface_render_method = "BLENDED"
  ...
  def create_composite_nodes(self):
    ...
    node_tree = bpy.data.node_groups.new(name="Composition." + self.suffix, type="CompositorNodeTree")
    ...
    renderlayers_node01 = nodes.get("Render Layers")
    if renderlayers_node01 is None:
      renderlayers_node01 = nodes.new("CompositorNodeRLayers")
      renderlayers_node01.name = "Render Layers"
    ...
    # Blender 5.0+ API: Use .interface.new_socket() instead of .inputs/.outputs
    alpha_group01.interface.new_socket(name="Image", in_out="INPUT", socket_type="NodeSocketColor")
    alpha_group01.interface.new_socket(name="Image", in_out="OUTPUT", socket_type="NodeSocketColor")
    alpha_group01.interface.new_socket(name="Alpha", in_out="OUTPUT", socket_type="NodeSocketFloat")
    ...
    alpha_group01_sHSVA = alpha_group01.nodes.new("CompositorNodeSeparateColor")
    alpha_group01_sHSVA.mode = "HSV"
    alpha_group01_sHSVA.location = (-200, 200)
    alpha_group01_cHSVA = alpha_group01.nodes.new("CompositorNodeCombineColor")
    alpha_group01_cHSVA.mode = "HSV"
    alpha_group01_cHSVA.location = (200, 200)
    alpha_group01_math = alpha_group01.nodes.new("ShaderNodeMath")
    alpha_group01_math.location = (0, 0)
    alpha_group01_math.operation = "GREATER_THAN"
    alpha_group01_math.inputs[1].default_value = 0.325
    # Links remain unchanged - they work with the new interface system
    ...
    # Only use one instance of the node group (remove unused alpha_convert_node02)
    alpha_convert_node01 = nodes.new("CompositorNodeGroup")
    alpha_convert_node01.node_tree = alpha_group01

    cryptomatte_node01 = nodes.new("CompositorNodeCryptomatteV2")
    cryptomatte_node01.matte_id = "Plane.shadow.{}".format(self.suffix)
    separate_hsva_node01 = nodes.new("CompositorNodeSeparateColor")
    separate_hsva_node01.mode = "HSV"
    invert_node01 = nodes.new("CompositorNodeInvert")
    screen_node01 = nodes.new("ShaderNodeMix")
    screen_node01.data_type = "RGBA"
    screen_node01.blend_type = "SCREEN"
    screen_node01.inputs[0].default_value = 1
    multiply_node01 = nodes.new("ShaderNodeMix")
    multiply_node01.data_type = "RGBA"
    multiply_node01.blend_type = "MULTIPLY"
    multiply_node01.inputs[0].default_value = 1
    colorramp_node01 = nodes.new("ShaderNodeValToRGB")
    ...
    rgb_node01 = nodes.new("CompositorNodeRGB")
    rgb_node01.outputs[0].default_value = (0, 0, 1, 1)
    rgb_node01.name = "BackgroundRGB"
    rgb_node01.label = "BackgroundRGB"
    rgb_node02 = nodes.new("CompositorNodeRGB")
    rgb_node02.outputs[0].default_value = (0, 0, 0, 0)
    rgb_node02.name = "BackgroundAlpha"
    rgb_node02.label = "BackgroundAlpha"
    alphaover_node01 = nodes.new("CompositorNodeAlphaOver")
    alphaover_node02 = nodes.new("CompositorNodeAlphaOver")
    alphaover_node01.inputs[4].default_value = True
    alphaover_node02.inputs[4].default_value = True

    links.new(switch_node07.outputs[0], group_output.inputs[0])
    links.new(switch_node06.outputs[0], switch_node07.inputs[1])
    links.new(switch_node05.outputs[0], switch_node06.inputs[1])
    links.new(switch_node04.outputs[0], switch_node05.inputs[1])
    links.new(switch_node03.outputs[0], switch_node04.inputs[1])
    links.new(switch_node02.outputs[0], switch_node03.inputs[1])
    links.new(switch_node01.outputs[0], switch_node02.inputs[1])

    links.new(renderlayers_node01.outputs[0], switch_node01.inputs[1])
    links.new(renderlayers_node01.outputs[0], alpha_convert_node01.inputs[0])
    links.new(alpha_convert_node01.outputs[0], alphaover_node01.inputs[1])

    links.new(renderlayers_node01.outputs[2], cryptomatte_node01.inputs[0])
    links.new(cryptomatte_node01.outputs[2], separate_hsva_node01.inputs[0])
    links.new(separate_hsva_node01.outputs[1], invert_node01.inputs[0])
    links.new(cryptomatte_node01.outputs[1], screen_node01.inputs[6])
    links.new(invert_node01.outputs[0], screen_node01.inputs[7])
    links.new(screen_node01.outputs[2], multiply_node01.inputs[7])
    links.new(alpha_convert_node01.outputs[1], multiply_node01.inputs[6])
    links.new(multiply_node01.outputs[2], colorramp_node01.inputs[0])
    links.new(colorramp_node01.outputs[0], alphaover_node02.inputs[1])

    links.new(rgb_node01.outputs[0], switch_node08.inputs[1])
    links.new(rgb_node02.outputs[0], switch_node08.inputs[2])
    links.new(switch_node08.outputs[0], alphaover_node01.inputs[0])
    links.new(switch_node08.outputs[0], alphaover_node02.inputs[0])

    links.new(alphaover_node01.outputs[0], switch_node01.inputs[2])
    links.new(alphaover_node01.outputs[0], switch_node02.inputs[2])
    links.new(alphaover_node01.outputs[0], switch_node03.inputs[2])
    links.new(alphaover_node02.outputs[0], switch_node04.inputs[2])
    links.new(alphaover_node02.outputs[0], switch_node05.inputs[2])
    links.new(alphaover_node01.outputs[0], switch_node06.inputs[2])
    links.new(alphaover_node01.outputs[0], switch_node07.inputs[2])
...
class RA2(BaseScene):
  ...
  props: WorldProperties = {"world_texture_path": "shared/generic_hdri.exr", "world_texture_name": "generic_hdri.exr"}
  ...
  def __init__(self):
    ...
    self.create_light()
    world = RA2_World(self.suffix, self.props)
    self.create_planes()
...
class RA2_INF(BaseScene):
  ...
  def __init__(self):
    ...
    self.create_light()
    world = RA2_INF_World(self.suffix, self.props)
    self.create_planes()
    self.create_composite_nodes()
    arrange_nodes([world.node_tree])
...
class TS(BaseScene):
  ...
  def __init__(self):
    ...
    self.create_light()
    world = TS_World(self.suffix, self.props)
    self.create_planes()
    self.create_composite_nodes()
    arrange_nodes([world.node_tree])
...
class RW(BaseScene):
  ...
  def __init__(self):
    ...
    self.create_light()
    world = RW_World(self.suffix, self.props)
    self.create_planes()
    self.create_composite_nodes()
    arrange_nodes([world.node_tree])
...
class RA1(BaseScene):
  ...
  def __init__(self):
    ...
    self.create_light()
    world = RA1_World(self.suffix, self.props)
    self.create_planes()
    self.create_composite_nodes()
    arrange_nodes([world.node_tree])
...
class RM(BaseScene):
  ...
  def __init__(self):
    ...
    self.create_light()
    world = RM_World(self.suffix, self.props)
    self.create_planes()
    self.create_composite_nodes()
    arrange_nodes([world.node_tree])
...
class D2K(BaseScene):
  ...
  def __init__(self):
    ...
    self.create_light()
    world = D2K_World(self.suffix, self.props)
    self.create_planes()
    self.create_composite_nodes()
    arrange_nodes([world.node_tree])
