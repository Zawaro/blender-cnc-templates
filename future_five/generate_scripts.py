...
def generate_scripts():
  ...
  for render_type in render_types:
    for render_engine in render_engines:
      ...
      sun_hide_render = "True" if render_engine == "CYCLES" else "False"  # Cycles uses Sky as the sun
      ...
      eevee_filter_size = str(constants.EEVEE_NEXT_FILTER_SIZE if render_type != "Shadow" else 0.01)
      ...
      # base += 'bpy.context.scene.compositing_node_group.nodes["Preview.Background"].inputs[0].default_value = ' + is_preview + '\r\n'
      base += 'bpy.context.scene.compositing_node_group.nodes["Object"].inputs[0].default_value = ' + is_object + "\r\n"
      base += 'bpy.context.scene.compositing_node_group.nodes["Buildup.Cycles"].inputs[0].default_value = ' + is_buildupCycles + "\r\n"
      base += 'bpy.context.scene.compositing_node_group.nodes["Buildup.Eevee"].inputs[0].default_value = ' + is_buildupEevee + "\r\n"
      base += 'bpy.context.scene.compositing_node_group.nodes["Shadow.Cycles"].inputs[0].default_value = ' + is_shadowCycles + "\r\n"
      base += 'bpy.context.scene.compositing_node_group.nodes["Shadow.Eevee"].inputs[0].default_value = ' + is_shadowEevee + "\r\n"
      base += 'bpy.context.scene.compositing_node_group.nodes["Preview.Cycles"].inputs[0].default_value = ' + is_previewCycles + "\r\n"
      base += 'bpy.context.scene.compositing_node_group.nodes["Preview.Eevee"].inputs[0].default_value = ' + is_previewEevee + "\r\n"
      if render_type == "Reset":
        base += 'bpy.context.scene.compositing_node_group.nodes["Alpha"].inputs[0].default_value = False\r\n'
  ...
  alpha_base = """import bpy

bpy.context.scene.compositing_node_group.nodes["Alpha"].inputs[0].default_value = False
...
bpy.context.scene.compositing_node_group.nodes["Alpha"].inputs[0].default_value = True
...
def get_base_scripts(render_engine=None):
  script = """import bpy

"""

  if render_engine:
    script += f'bpy.context.scene.render.engine = "{render_engine}"\n\n'

  script += """suffix = "RA2.INF"
