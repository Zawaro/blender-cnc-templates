...
def generate_scripts():
  ...
  for render_type in render_types:
    for render_engine in render_engines:
      ...
      sun_hide_render = (
        "True"
        if render_engine == "BLENDER_EEVEE" and render_type == "Shadow"
        else "False"
      )
      sun_shadow_hide_render = (
        "False"
        if render_engine == "BLENDER_EEVEE" and render_type == "Shadow"
        else "True"
      )
      ...
      base += (
        'bpy.data.objects["Plane.holdout." + suffix].hide_render = '
        + str(bool(props[0]))
        + "\r\n"
      )
      base += (
        'bpy.data.objects["Plane.holdout2." + suffix].hide_render = '
        + str(bool(props[1]))
        + "\r\n"
      )
      base += (
        'bpy.data.objects["Plane.shadow." + suffix].hide_render = '
        + str(bool(props[2]))
        + "\r\n"
      )
      base += (
        'bpy.data.objects["Plane.shadow2." + suffix].hide_render = '
        + str(bool(props[3]))
        + "\r\n"
      )
      base += (
        'bpy.data.objects["Plane.blue." + suffix].hide_render = '
        + str(bool(props[4]))
        + "\r\n"
      )
      base += (
        'bpy.data.objects["Plane.grey." + suffix].hide_render = '
        + str(bool(props[5]))
        + "\r\n"
      )
      base += (
        'bpy.data.objects["Plane.ambient." + suffix].hide_render = '
        + str(bool(props[6]))
        + "\r\n"
      )
      base += (
        'bpy.data.objects["Sun." + suffix].hide_render = ' + sun_hide_render + "\r\n"
      )
      base += (
        'bpy.data.objects["Sun.shadow." + suffix].hide_render = '
        + sun_shadow_hide_render
        + "\r\n"
      )

      cycles_filter_width = str(
        constants.CYCLES_FILTER_WIDTH if render_type != "Shadow" else 0.01
      )
      eevee_filter_size = str(
        constants.EEVEE_FILTER_SIZE if render_type != "Shadow" else 0.01
      )
      ...
      base += (
        "bpy.context.scene.render.use_single_layer = " + str(bool(props[7])) + "\r\n"
      )

      # is_preview = 'True' if render_type == 'Preview' else 'False'
      is_object = "True" if render_type == "Object" else "False"
      is_buildupCycles = (
        "True" if render_type == "Buildup" and render_engine == "CYCLES" else "False"
      )
      is_buildupEevee = (
        "True"
        if render_type == "Buildup" and render_engine == "BLENDER_EEVEE"
        else "False"
      )
      is_shadowCycles = (
        "True" if render_type == "Shadow" and render_engine == "CYCLES" else "False"
      )
      is_shadowEevee = (
        "True"
        if render_type == "Shadow" and render_engine == "BLENDER_EEVEE"
        else "False"
      )
      is_previewCycles = (
        "True" if render_type == "Preview" and render_engine == "CYCLES" else "False"
      )
      is_previewEevee = (
        "True"
        if render_type == "Preview" and render_engine == "BLENDER_EEVEE"
        else "False"
      )
      ...
      base += (
        'bpy.context.scene.node_tree.nodes["Object"].check = ' + is_object + "\r\n"
      )
      base += (
        'bpy.context.scene.node_tree.nodes["Buildup.Cycles"].check = '
        + is_buildupCycles
        + "\r\n"
      )
      base += (
        'bpy.context.scene.node_tree.nodes["Buildup.Eevee"].check = '
        + is_buildupEevee
        + "\r\n"
      )
      base += (
        'bpy.context.scene.node_tree.nodes["Shadow.Cycles"].check = '
        + is_shadowCycles
        + "\r\n"
      )
      base += (
        'bpy.context.scene.node_tree.nodes["Shadow.Eevee"].check = '
        + is_shadowEevee
        + "\r\n"
      )
      base += (
        'bpy.context.scene.node_tree.nodes["Preview.Cycles"].check = '
        + is_previewCycles
        + "\r\n"
      )
      base += (
        'bpy.context.scene.node_tree.nodes["Preview.Eevee"].check = '
        + is_previewEevee
        + "\r\n"
      )
      ...
      filename = "{}.Render.{}.txt".format(
        get_engine_output_str(render_engine), render_type
      )
